...
export async function createCard(cardInfo: { name: string; type: cardRepository.TransactionTypes; employeeId: number }) {
...
export async function getBalance(cardInfo: { cardId: number }) {
  ...
  const rechargeAmount = recharges.reduce((acc: any, cur: { amount: number; }) => acc + cur.amount, 0);
  const paymentAmount = payments.reduce((acc: any, cur: { amount: number; }) => acc + cur.amount, 0);
